...
// 这个模块的主要任务是讲cpp结构的mpu_9250驱动和c结构的官方dmp库嫁接
// 自己算的欧拉角实在不准，只好用官方的dmp来减少工程量
// http://blog.csdn.net/ikevin/article/details/53352702

//
// 这里是把cpp接口返回给dmp
extern __mpu9250 IMU;		// CS1, 50KHz
...
int writeBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t* data) {
	...
	for (i = 0; i < length; i++)
		IMU.write_Reg(regAddr + i, data[i]);
...
int8_t readBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *data){
	...
	char byte[length+1] = { 0 };
	int i;

	for (i = 0; i < length; i++)
		byte[i] = IMU.read_Reg(regAddr +i);
